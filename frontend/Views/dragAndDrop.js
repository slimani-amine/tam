...
const debouncedChangeTask = debounce(changeTask, 700);
...
export const dragAndDrop = async (draggables, draggables2, droppables, droppables2) => {
  ...
  droppables.forEach((zone) => {
    zone.addEventListener("dragover", async (e) => {
      ...
      status && taskId && await debouncedChangeTask(taskId, null, null, status, null, null)
  ...
  droppables2.forEach(async (zone) => {
    zone.addEventListener("dragover", async (e) => {
      ...
      if (status !== "" && taskId !== 0) {
        const changed = debouncedChangeTask(taskId, null, null, status, null, null, null)
